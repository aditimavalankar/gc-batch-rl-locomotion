...
targetStartPos = [1,0,0.25]
targetStartOrn = [0.5,0.5,0.5,0.5]
...
class MinitaurBulletEnv(gym.Env):
  ...
  def _reset(self):
    if self._hard_reset:
      ...
      self._targetId = self._pybullet_client.loadURDF("%s/target.urdf" % self._urdf_root, targetStartPos, targetStartOrn)
      ...
      self._pybullet_client.resetBasePositionAndOrientation(self._targetId,targetStartPos,targetStartOrn)
    ...
    # THIS IS A CHANGE I MADE! WE DO NOT WANT THE SAME OBS AT EACH RESET SO WE ADD RANDOM NOISE
    # AND RESET THE NOISE TO ZERO AFTER INITIALIZING THE SIMULATION!
    self._observation_noise_stdev = 0.1
    self.current_s = self._noisy_observation()
    self.current_o = np.copy(self.get_minitaur_position())
  ...
  def _step(self, action):
    ...
    reward = self._reward()
    done = self._termination() or self.success()
    new_s = self._noisy_observation()
    new_o = np.copy(self.get_minitaur_position())
    # print('Old s:', self.current_s)
    # print('New s:', new_s)
    # print('Old o:', self.current_o)
    # print('New o:', new_o)
    # print('Action:', action)
    # fp = open('IDM_Minitaur_data_trained_left.pkl', 'a+b')
    # pickle.dump([self.current_s, self.current_o, new_s, new_o,
    #              unprocessed_action], fp)
    # fp.close()
    self.current_s = new_s
    self.current_o = new_o
    return np.array(self.current_s), reward, done, {}
  ...
  def get_minitaur_position(self):
    """Get the minitaur's location.

    Returns:
      A numpy array of the 3-D coordinates of the position of the Minitaur.
    """
    return np.array(self.minitaur.GetBasePosition())

  def get_minitaur_rotation_angle(self):
    """Get the angle by which the minitaur has been rotated."""
    return self.minitaur.angle

  def success(self):
    """Did the Minitaur reach the target?

    Returns:
      A boolean value indicating if the Minitaur succeeded or not.
    """
    points = self._pybullet_client.getContactPoints(self._targetId,
                                                    self.minitaur.quadruped)
    target_pos = self.get_target_position()
    robot_pos = self.get_minitaur_position()
    distance = np.sqrt((target_pos[0] - robot_pos[0]) ** 2 +
                       (target_pos[1] - robot_pos[1]) ** 2)
    return len(points) > 0 or distance <= self.distance_threshold
  ...
  def set_render(self):
      """Sets the render parameter of the env to True for visualization."""
