...
class Minitaur(object):
  ...
  def Reset(self, reload_urdf=True):
    ...
    # THESE LINES HAVE BEEN ADDED BY ME, THEY ARE NOT PART OF THE ORIGINAL CODE
    # self.angle = np.random.random() * 2 * np.pi
    self.angle = 0
    prev_quat = INIT_ORIENTATION
    rot_quat = [0,
                0,
                np.sin(self.angle / 2),
                np.cos(self.angle / 2)]
    # rot_quat = [0, 0, 0, 1]
    modified_quat = [prev_quat[0] * rot_quat[3] - prev_quat[1] * rot_quat[2],
                     prev_quat[1] * rot_quat[3] + prev_quat[0] * rot_quat[2],
                     prev_quat[2] * rot_quat[3] + prev_quat[3] * rot_quat[2],
                     prev_quat[3] * rot_quat[3] - prev_quat[2] * rot_quat[2]]
    self._pybullet_client.resetBasePositionAndOrientation(
        self.quadruped, INIT_POSITION, modified_quat)
    self._pybullet_client.resetBaseVelocity(self.quadruped, [0, 0, 0],
                                            [0, 0, 0])
    self.ResetPose(add_constraint=False)
    ###########################################
