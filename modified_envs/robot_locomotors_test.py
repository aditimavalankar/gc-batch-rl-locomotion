...
class WalkerBase(MJCFBasedRobot):
  ...
  def calc_state(self):
    ...
    # print('Target:', self.walk_target_x, self.walk_target_y)
    self.walk_target_theta = np.arctan2(self.walk_target_y - self.body_xyz[1],
                                        self.walk_target_x - self.body_xyz[0])

    """Commented below line and replaced it the one below it, because we do not
    want the Humanoid to walk absolutely straight, we just want it to go to the
    right. The reward below (commented) is more like that for training a goal-
    conditioned policy."""
    # self.walk_target_dist = np.linalg.norm(
    #     [self.walk_target_y - self.body_xyz[1], self.walk_target_x - self.body_xyz[0]])
    self.walk_target_dist = abs(self.walk_target_x - self.body_xyz[0])
    angle_to_target = self.walk_target_theta - yaw
    # print('Angle to target:', np.degrees(angle_to_target))
...
class Humanoid(WalkerBase):
  ...
  def __init__(self):
    ...
    #                     obs_dim=44,
    #                     power=0.41)
    """NOTE: I have changed the obs dim to 43, as I am replacing the sin and cos
    of target angle with yaw, since we do not need a goal-conditioned policy."""
    WalkerBase.__init__(self,
                        'humanoid_symmetric.xml',
                        'torso',
                        action_dim=17,
  ...
  def set_target_position(self, target_x, target_y):
    ...
    if (self.target):
      ...
      #	p.removeBody(b)
      self._p.resetBasePositionAndOrientation(self.target.bodies[0],
